...
global_fire_danger1=None
global_fire_danger2=None
...
#temperature_c = (5/9) * (temperature_f - 32)
			...
			fire_hazard_index2=calculate_fire_danger_index(global_temperature2,global_humidity2)
# 스위치 값이 변경될 때까지 계속 읽는다.
			switch_value = read_switch()
            # 스위치가 눌리면 부저를 꺼야 합니다.
			if switch_value == 1:
				break
			if fire_hazard_index == "Red" or fire_hazard_index2 == "Red":
                # 주파수 값을 설정한다.
...
def led():
	try:
		...
		while True:
			time.sleep(2.0)
			fire_hazard_index1=calculate_fire_danger_index(global_temperature1,global_humidity1)
			fire_hazard_index2=calculate_fire_danger_index(global_temperature2,global_humidity2)
